...
Record evidence for every disposition. Uncertainty is not evidence; investigate
or defer it.
...
3. Run targeted checks, then the repository's required preflight.
4. Use the repository tracker or finding-capture flow for deferrals. If none
   exists, report the deferred work without inventing a tracker.
5. Publish through the repository's PR workflow. Reply with durable evidence:
   the fixing PR or commit, existing fix, deferral id, or rejection reason.
6. Resolve threads only when repository policy permits and the disposition has
