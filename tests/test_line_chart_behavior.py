...
from textcharts import ASCIIChartOptions, ASCIILineChart, LinePoint
...
def test_empty_data():
    assert "No data" in ASCIILineChart(points=[]).render()
...
def test_trend_not_shown_when_disabled():
    points = [LinePoint("S", i, float(i * 10)) for i in range(1, 8)]
    no_trend = ASCIILineChart(points=points, show_trend=False, options=_opts()).render()
    with_trend = ASCIILineChart(points=points, show_trend=True, options=_opts()).render()
    # Trend version should have more dots
    assert with_trend.count(".") >= no_trend.count(".")


def test_compute_trend_linear():
    chart = ASCIILineChart(points=[LinePoint("S", 1, 1.0)], options=_opts())
...
def test_multi_series_renders_legend():
    ...
    result = ASCIILineChart(points=points, options=_opts()).render()
